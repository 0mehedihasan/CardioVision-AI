...
function App() {
  ...
  const patientId = useMemo(() => `CV-${new Date().getFullYear()}-001`, []);
  ...
  const askQuestion = (text) => {
    ...
    setConversation((previous) => [
      ...
      {
        role: "assistant",
        text: "This case-level response will be generated by the local medical language model once the multimodal inference pipeline is connected.",
        evidence: analysisComplete
          ? {
              finding: "Coronary anatomy — potential abnormality",
              confidence: "87.4%",
              context: [
                clinicalData.age ? `Age ${clinicalData.age}` : null,
                clinicalData.hypertension ? "Hypertension: yes" : null,
              ].filter(Boolean),
            }
          : null,
      },
    ...
    <div className="cv-shell">
      <header className="cv-header">
        ...
        <div className="cv-header-actions">
          <button className="cv-header-button" onClick={() => scrollToSection("case")}>
            New case
          </button>
      ...
      <div className="cv-body">
        <aside className="cv-sidebar">
          ...
          {navItems.map((item) => (
            <button
              key={item.id}
              className={`cv-nav-item ${activeSection === item.id ? "active" : ""}`}
              onClick={() => scrollToSection(item.id)}
            >
          ...
          <div className="cv-local-card">
            ...
            <div>
              ...
              <div className="cv-local-text">Patient data remains on this device.</div>
            </div>
          </div>
        </aside>
        ...
        <main className="cv-main">
          <section className="cv-hero">
            ...
            <p>
              Analyze clinical information, echocardiography, CCTA, and ECG
              data within a unified patient case.
            </p>
          </section>
            ...
            <div className="cv-analysis-bar">
              <div>
                ...
                <div className="cv-analysis-subtitle">
                  {uploadedCount} imaging modalit{uploadedCount === 1 ? "y" : "ies"} provided
                  {hasClinicalData ? " · Clinical data available" : " · No clinical data yet"}
                </div>
              </div>
          ...
          <section id="results" className="cv-section">
            <div className="cv-section-head">
              <div>
                <div className="cv-section-index">02 / Multimodal analysis</div>
                <h2>Case intelligence</h2>
                <p>
                  A unified workspace for modality-specific inference and
                  multimodal interpretation.
                </p>
              </div>

              <div className={`cv-status-chip ${analysisComplete ? "complete" : ""}`}>
                <span />
            ...
            {!analysisComplete && !isAnalyzing && (
              <div className="cv-empty-analysis">
                ...
                <p>
                  Add patient information or imaging data above, then run the
                  analysis pipeline.
                </p>
                <button className="cv-secondary-button" onClick={() => scrollToSection("case")}>
            ...
            {analysisComplete && (
              <div className="cv-results">
                ...
                {activeResult === "overview" && (
                  <OverviewResult files={files} clinicalData={clinicalData} />
                )}
                {activeResult === "echo" && <ModalityResult
                  file={files.echo}
                  label="Echocardiography"
                ...
                />}
                {activeResult === "ccta" && <ModalityResult
                  file={files.ccta}
                  label="CCTA"
              ...
              <section id="explainability" className="cv-section">
                <div className="cv-section-head">
                  <div>
                    ...
                    <h2>See why the model predicts</h2>
                    <p>
                      Model explanations connect predictions with relevant
                      image regions and anatomical structures.
                    </p>
                  </div>
                </div>
                ...
                <div className="cv-xai-grid">
                  <div className="cv-xai-image">
                    <div className="cv-image-placeholder cv-heatmap">
                      ...
                      <span className="cv-image-tag">Grad-CAM</span>
                    </div>
                    <div className="cv-image-caption">
                      <strong>Prediction attribution</strong>
                      <span>Regions contributing to the selected prediction</span>
                    </div>
                  </div>

                  <div className="cv-xai-content">
                    <span className="cv-card-kicker">Model explanation</span>
                    <h3>Coronary abnormality</h3>
                    ...
                    <div className="cv-finding-list">
                      <Finding title="Coronary anatomy" value="Relevant region detected" />
                      <Finding title="Cardiac structure" value="Left ventricular region" />
                      <Finding title="Model attention" value="High regional contribution" />
                    </div>

                    <div className="cv-xai-note">
                      Explainability outputs show model attribution and should
                      not be interpreted as independent clinical evidence.
                    </div>
                  </div>
                </div>
                ...
                <div className="cv-segmentation-card">
                  <div>
                    <span className="cv-card-kicker">Structural analysis</span>
                    <h3>Cardiovascular segmentation</h3>
                    <p>
                      Predicted anatomical structures can be visualized
                      alongside the original imaging data.
                    </p>
                  </div>
                  ...
                  <div className="cv-segmentation-visual">
                    ...
                    <div className="cv-segmentation-legend">
                      <span><i /> Coronary vessel</span>
                      <span><i /> Cardiac structure</span>
                      <span><i /> Segmentation region</span>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section id="assistant" className="cv-section cv-assistant-section">
                <div className="cv-section-head">
                  <div>
                    ...
                    <p>
                      Query the local medical language model using the
                      available case context and model-generated findings.
                    </p>
                  </div>
                ...
                <div className="cv-assistant">
                  <div className="cv-assistant-context">
                    <div className="cv-assistant-context-header">
                      ...
                      <span className="cv-context-status">Loaded</span>
                    </div>

                    <div className="cv-context-items">
                      <ContextItem label="Clinical" active={hasClinicalData} />
                      <ContextItem label="Echocardiography" active={Boolean(files.echo)} />
                      <ContextItem label="CCTA" active={Boolean(files.ccta)} />
                      <ContextItem label="ECG" active={Boolean(files.ecg)} />
                      <ContextItem label="AI findings" active={analysisComplete} />
                    </div>
                  </div>
                  ...
                  <div className="cv-chat">
                    {conversation.length === 0 ? (
                      <div className="cv-chat-empty">
                        ...
                        <p>
                          Ask the local model to summarize, explain, or reason
                          over the available case information.
                        </p>

                        <div className="cv-question-suggestions">
                          <button onClick={() => askQuestion("What are the main findings in this case?")}>
                            Summarize the findings
                          </button>
                          <button onClick={() => askQuestion("What findings require further attention?")}>
                            What needs attention?
                          </button>
                          <button onClick={() => askQuestion("Explain the imaging findings.")}>
                            Explain the imaging
                          </button>
                        </div>
                      </div>
        ...
        <div className="cv-checkbox-grid">
          <label className="cv-checkbox">
            <input
              ...
              onChange={(event) => onChange("diabetes", event.target.checked)}
            />
            <span>Diabetes</span>
          </label>
          ...
          <label className="cv-checkbox">
            <input
              ...
              onChange={(event) => onChange("hypertension", event.target.checked)}
            />
  ...
  return (
    ...
    >
      ...
      <div className="cv-modality-info">
        <div className="cv-modality-title">{modality.label}</div>
        <div className="cv-modality-description">
          {file ? file.name : modality.description}
        </div>
        {!file && <div className="cv-modality-formats">{modality.formats}</div>}
        {file && (
      ...
      <div className="cv-modality-actions">
        {file ? (
          <>
            ...
            <button className="cv-ghost-button" onClick={() => inputRef.current?.click()}>
              Replace
            </button>
...
function AnalysisLoader() {
  return (
    <div className="cv-analysis-loader">
      ...
      <div>
        <h3>Running multimodal analysis</h3>
        <p>
          Processing the available patient data through modality-specific
          models and preparing the fusion context.
        </p>
      </div>

      <div className="cv-loader-steps">
        <LoaderStep number="01" text="Preprocessing" active />
        <LoaderStep number="02" text="Modality inference" active />
        <LoaderStep number="03" text="Multimodal reasoning" />
        <LoaderStep number="04" text="Explainability" />
      </div>
    </div>
  );
}
...
function OverviewResult({ files, clinicalData }) {
  return (
    <div className="cv-overview">
      <div className="cv-summary-card">
        <div className="cv-summary-main">
          <span className="cv-card-kicker">Primary model finding</span>
          <h3>Cardiovascular abnormality detected</h3>
          <p>
            The available case information indicates findings that warrant
            further clinical review. This result represents a research model
            output.
          </p>
        </div>
      ...
      <div className="cv-metric-grid">
        <Metric label="Clinical risk" value="Elevated" tone="warning" />
        <Metric label="Echo finding" value={files.echo ? "Analyzed" : "Awaiting input"} tone={files.echo ? "" : "muted"} />
        <Metric label="CCTA finding" value={files.ccta ? "Analyzed" : "Awaiting input"} tone={files.ccta ? "" : "muted"} />
        <Metric label="ECG" value={files.ecg ? "Analyzed" : "Awaiting input"} tone={files.ecg ? "" : "muted"} />
      </div>

      <div className="cv-findings-grid">
        <div className="cv-finding-card">
          <span className="cv-card-kicker">Key findings</span>
          <Finding title="Coronary anatomy" value="Potential abnormality" />
          <Finding title="Cardiac function" value="Requires review" />
          <Finding
            title="Clinical profile"
            value={clinicalData.hypertension ? "Hypertension reported" : "Available data reviewed"}
          />
        </div>

        <div className="cv-finding-card cv-next-step">
          <span className="cv-card-kicker">Next step</span>
          <h3>Review model evidence</h3>
          <p>
            Inspect modality-specific outputs and explainability
            visualizations before drawing any clinical conclusion.
          </p>
        </div>
      </div>
    </div>
...
function ModalityResult({ file, label, metricLabel, metricValue, confidence, detail }) {
  return (
    <div className="cv-modality-result">
      <div className="cv-result-visual">
        <div className="cv-result-visual-image">
          ...
          <span className="cv-image-tag">Model output</span>
        </div>
        <div className="cv-image-caption">
          <strong>{label} analysis</strong>
          <span>{file ? `Analyzed: ${file.name}` : `No ${label.toLowerCase()} file provided`}</span>
        </div>
      </div>

      <div className="cv-result-detail">
        <span className="cv-card-kicker">{label} model</span>
        <h3>Structural analysis</h3>
        <p>{detail}</p>

        <div className="cv-result-stat-row">
          <Metric label={metricLabel} value={metricValue} />
          <Metric label="Model confidence" value={confidence} />
        </div>
      </div>
    </div>
  );
...
function ClinicalResult({ clinicalData }) {
  return (
    <div className="cv-clinical-result">
      <div>
        <span className="cv-card-kicker">Clinical model</span>
        <h3>Patient-level clinical assessment</h3>
        <p>
          Structured clinical variables are processed by the clinical
          prediction model and contribute to the multimodal case
          representation.
        </p>
      </div>

      <div className="cv-clinical-values">
        <Metric label="Age" value={clinicalData.age || "Not provided"} tone={clinicalData.age ? "" : "muted"} />
        <Metric label="Sex" value={clinicalData.sex || "Not provided"} tone={clinicalData.sex ? "" : "muted"} />
        <Metric
          label="Blood pressure"
          value={clinicalData.bloodPressure || "Not provided"}
          tone={clinicalData.bloodPressure ? "" : "muted"}
        />
        <Metric
          label="Heart rate"
          value={clinicalData.heartRate ? `${clinicalData.heartRate} bpm` : "Not provided"}
          tone={clinicalData.heartRate ? "" : "muted"}
        />
      </div>
    </div>
  );
...
function Finding({ title, value }) {
  return (
    <div className="cv-finding">
      <div className="cv-finding-check">✓</div>
      <div>
        <strong>{title}</strong>
        <span>{value}</span>
      </div>
      ...
      {message.evidence && (
        <div className="cv-message-evidence">
          <div className="cv-evidence-row">
            <span>Evidence</span>
            <strong>{message.evidence.finding}</strong>
          </div>
          <div className="cv-evidence-row">
            <span>Model confidence</span>
            <strong>{message.evidence.confidence}</strong>
          </div>
          {message.evidence.context.length > 0 && (
            <div className="cv-evidence-row">
              <span>Clinical context</span>
              <strong>{message.evidence.context.join(" · ")}</strong>
            </div>
          )}
        </div>
      )}
